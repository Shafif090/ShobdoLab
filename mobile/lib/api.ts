...
async function refreshStoredAuthSession() {
  ...
  if (!refreshAuthPromise) {
    refreshAuthPromise = (async () => {
      ...
      await saveAuthSession(refreshed, { preserveSessionExpiresAt: true });
      return refreshed;
    })().finally(() => {
...
export async function getExerciseMeta(token: string) {
  return request<ExerciseMetaResponse>("/v1/exercise/meta", {
    token,
  });
}
...
export async function getWordDetail(token: string, wordId: string | number) {
  return request<WordDetailResponse>(`/v1/words/${wordId}`, {
    token,
  });
}
...
export async function getCurrentSet(token: string) {
  return request<LearningSetResponse>("/v1/learn/current-set", {
    token,
  });
}
...
export async function getQuizSession(token: string, sessionId: string) {
  return request<QuizSessionResponse>(`/v1/quiz/${sessionId}`, {
    token,
  });
}
...
export async function getQuizResult(token: string, sessionId: string) {
  return request<QuizResultResponse>(`/v1/quiz/${sessionId}/result`, {
    token,
  });
}
